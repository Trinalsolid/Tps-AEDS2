...
class Hash {
    ...
    public boolean inserir(Filme elemento) {
       boolean resp = false;
       int soma = 0;
       for(int i = 0; i < elemento.getTituloOriginal().length(); i++){
            soma = soma + (int) elemento.getTituloOriginal().charAt(i);
       }
       if (elemento != NULO) {
          int pos = h(soma);
          if (tabela[pos] == NULO) {
    ...
    public static void main(String[] args) throws Exception{
        ...
        do{
            ...
            if(isFim(entrada2) == false){
                ...
                testemp = hashcomrehash.pesquisar(entrada2);
                if(testemp == false){
