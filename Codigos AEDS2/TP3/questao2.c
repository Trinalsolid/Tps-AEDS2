...
void Selecao( Lista *lista, int menor, int maior){
   int aux, i;
   Filme temp;
   if (menor < maior){
      aux = menor;
      for (i = menor + 1; i <= maior; i++) {
         if (strcmp(lista->filmes[i].tituloOriginal, lista->filmes[aux].tituloOriginal) < 0){
             aux = i;
         } 
      }
      swap(lista, menor, aux);
      Selecao(lista, menor+1, maior);
    }
}
