...
class Filme{
    ...
    public void ler(String entrada) throws Exception{
        ...
        Arq.openRead("C:\\Users\\WazX\\Desktop\\Filmes\\"+entrada);
...
class Lista {
    ...
    public void QuickSort(int esq, int dir) {
        ...
        while (i <= j) {
        while (filmes[i].getSituacao().charAt(0) < pivo.getSituacao().charAt(0)){
        i++;
        comparacoes++; 
        } 
        while (filmes[j].getSituacao().charAt(0) > pivo.getSituacao().charAt(0)){
            j--;
            comparacoes++;
        } 
        if (i <= j) {
    ...
    public void AlfabeticSort(){
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            for (int j = (i + 1); j < n && filmes[menor].getSituacao().charAt(0) == filmes[j].getSituacao().charAt(0) ; j++){
                comparacoes ++;
