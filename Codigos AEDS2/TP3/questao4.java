...
class Lista {
    ...
    public void heapsort() {
     ...
     public void construir(int tamHeap){
        for(int i = tamHeap; i > 1 && array[i].getGenre().compareTo(array[i/2].getGenre())>0; i /= 2){
           swap(i, i/2);
        }
     }
    ...
    public int getMaiorFilho(int i, int tamHeap){
        int filho;
        if (2*i == tamHeap || array[2*i].getGenre().compareTo(array[2*i+1].getGenre())>0){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
